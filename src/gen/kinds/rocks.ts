...
export async function createRocks(
  ...
  // Pinnacles: the landmarks. Tall eroded spires, metres above everything
  // else, so the reef has something to be small against.
  const SPIRES = 3;
  const spireStart = variants.length;
  for (let v = 0; v < SPIRES; v++) {
    variants.push({
      patches: [
        {
          segU: hi ? 110 : 44,
          ...
          params: [
            ...
            rng.range(-50, 50),
            rng.range(-50, 50),
            // Many ledges up a tall column: eroded, not a smooth tusk.
            rng.range(2.6, 3.6),
          ],
        },
      ],
  ...
  const y0 = (x: number, z: number) => ctx.groundY(x, z);
  ...
  const place = (
    ...
    instances.push({
      pos: [x, y, z],
      scale,
      rot: quatUpYaw(landmark === null ? up : [0, 1, 0], yaw),
      color: tint(),
  ...
  for (const c of ctx.clusters.slice(0, rng.int(2, 3))) {
    // Off the edge of the reef, so it stands clear against open water.
    const a = rng.range(0, Math.PI * 2);
    const d = c.radius * rng.range(0.85, 1.25);
    const x = c.x + Math.cos(a) * d;
    const z = c.z + Math.sin(a) * d;
    if (ctx.terrain.normalAt(x, z)[1] < 0.8) {
      continue;
    }
    const scale = rng.range(2.2, 3.2) * (c.rank === 0 ? 1.15 : 1);
    place(
      x,
      z,
      scale,
      ...
      false,
      false,
      spireStart + rng.int(0, SPIRES - 1),
    );
    ctx.occupied.add(x, z, scale * 0.7);
    // A column of spheres so fish, coral and the camera all know its height.
    for (const [k, rr] of [
      [0.2, 0.75],
      [1.1, 0.62],
      [2.0, 0.5],
      ...
      ctx.obstacles.push({
        center: [x, y0(x, z) + scale * k, z],
        radius: scale * rr,
      });
    }
    landmarkRocks(x, z, scale, scale * 0.75);
    ctx.landmarks.push({
      kind: 'pinnacle',
      x,
      y: y0(x, z),
